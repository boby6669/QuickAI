      ...
      <form onSubmit={onSubmitHandler} className='bg-white w-full max-w-lg p-4 rounded-lg border border-gray-200'>
        ...
        <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
          {blogCategories().map((item)=>(
            <span onClick={() => setSelectedCategory(item)} key={item} className={`text-xs px-4 py-1 border
                     border-gray-300 rounded-full cursor-pointer ${selectedCategory === item ? 'bg-purple-50 text-purple-700 border-gray-500' : 'text-gray-500 border-gray-300'}`}>{item}</span>
          ))}
        </div>
        <br />
