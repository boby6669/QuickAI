           ...
           <p className='mt-4 text-sm font-medium'>Article Length </p>
            ...
            <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
                {articleLength().map((item, index)=>(
                  <span onClick={()=> setSelectedLength(item)} key={index} className={`text-xs px-4 py-1 border
                     border-gray-300 rounded-full cursor-pointer ${selectedLength.text === item.text ? 'bg-blue-50 text-blue-700 border border-gray-500' : 'text-gray-500 border-gray-300'}`}>{item.text}</span>
                )) }
            </div>
            <br />
